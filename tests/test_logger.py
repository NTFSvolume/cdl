...
def test_export_logs(tmp_path: Path) -> None:
    ...
    with log.setup_logging(log_file):
        for line in TEXT.splitlines():
            log.logger.debug(line)

        content = log.export_logs().decode("utf8")
        with pytest.raises(RuntimeError):
            _ = log.export_logs(size_limit=1)
...
class TestBorrowLogger:
    def test_handlers_swapped_temporarily(self) -> None:
        ...
        log.logger.addHandler(root_handler)
        ...
        try:
            ...
            with log.borrow_logger("third_party"):
                assert other.handlers == [root_handler]

            assert other.handlers == [original_handler]
        finally:
            log.logger.removeHandler(root_handler)
    ...
    def test_level_and_propagate_restored(self) -> None:
        ...
        with log.borrow_logger("third_party_restore", level=logging.DEBUG):
    ...
    def test_exception_inside_block_restores_state(self) -> None:
        ...
        with pytest.raises(RuntimeError):
            with log.borrow_logger("third_party_exc", level=logging.INFO):
